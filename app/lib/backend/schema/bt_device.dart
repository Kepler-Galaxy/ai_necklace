...
Future<DeviceType?> getTypeOfBluetoothDevice(BluetoothDevice device) async {
  ...
  if (device.servicesList
      .where((s) => s.uuid == Guid(friendServiceUuid))
      .isNotEmpty) {
    ...
    final hasImageStream = device.servicesList
        ...
        .any((c) =>
            ...
            imageDataStreamCharacteristicUuid.toLowerCase());
    deviceType = hasImageStream ? DeviceType.openglass : DeviceType.AudioFoxxy;
  } else if (device.servicesList
      .where((s) => s.uuid == Guid(frameServiceUuid))
      .isNotEmpty) {
...
class BTDeviceStruct {
  ...
  factory BTDeviceStruct.fromJson(Map<String, dynamic> json) {
    ...
    if (fwver != null) {
      if (fwver.firstOrNull is int) {
        ...
        fwver = (fwver.firstOrNull as String)
            .split('.')
            .map((e) => int.parse(e.replaceFirst('v', '')))
            .toList();
      } else {
        fwver = null;
      }
  ...
  Map<String, dynamic> toJson({bool fwverAsString = false}) => {
        ...
        'name': name,
        'rssi': rssi,
        'fwver': fwverAsString ? fwver?.join('.') : fwver,
        'type': type?.name
      };
}
...
class DeviceInfo {
  ...
  DeviceInfo(this.modelNumber, this.firmwareRevision, this.hardwareRevision,
      this.manufacturerName, this.type);

  static Future<DeviceInfo> getDeviceInfo(
      BTDeviceStruct? device, DeviceConnection? conn) async {
    if (device == null) {
      return DeviceInfo(
          'Unknown', 'Unknown', 'Unknown', 'Unknown', DeviceType.AudioFoxxy);
    }
    if (conn == null) {
      return DeviceInfo(
          'Unknown', 'Unknown', 'Unknown', 'Unknown', DeviceType.AudioFoxxy);
    ...
    if (device.type == DeviceType.AudioFoxxy) {
  ...
  static Future<DeviceInfo> _getDeviceInfoFromFriend(
      BTDeviceStruct? device, DeviceConnection conn) async {
    var modelNumber = 'AudioFoxxy';
    var firmwareRevision = '1.0.2';
    var hardwareRevision = 'Seeed Xiao BLE Sense';
    var manufacturerName = 'Kepler Galaxy';

    if (device == null) {
      return DeviceInfo(modelNumber, firmwareRevision, hardwareRevision,
          manufacturerName, DeviceType.AudioFoxxy);
    }

    var deviceInformationService =
        await conn.getService(deviceInformationServiceUuid);
    if (deviceInformationService != null) {
      // var modelNumberCharacteristic = conn.getCharacteristic(
      //     deviceInformationService, modelNumberCharacteristicUuid);
      // if (modelNumberCharacteristic != null) {
      //   modelNumber =
      //       String.fromCharCodes(await modelNumberCharacteristic.read());
      // }

      var firmwareRevisionCharacteristic = conn.getCharacteristic(
          deviceInformationService, firmwareRevisionCharacteristicUuid);
      if (firmwareRevisionCharacteristic != null) {
        firmwareRevision =
            String.fromCharCodes(await firmwareRevisionCharacteristic.read());
      }

      var hardwareRevisionCharacteristic = conn.getCharacteristic(
          deviceInformationService, hardwareRevisionCharacteristicUuid);
      if (hardwareRevisionCharacteristic != null) {
        hardwareRevision =
            String.fromCharCodes(await hardwareRevisionCharacteristic.read());
      }

      // var manufacturerNameCharacteristic = conn.getCharacteristic(
      //     deviceInformationService, manufacturerNameCharacteristicUuid);
    ...
    var type = DeviceType.AudioFoxxy;
    if (device.type == DeviceType.openglass) {
      ...
      if (friendService != null) {
        var imageCaptureControlCharacteristic = conn.getCharacteristic(
  ...
  static Future<DeviceInfo> _getDeviceInfoFromFrame(
      BTDeviceStruct? device, FrameDeviceConnection conn) async {
    await conn.init();
    return DeviceInfo(
      conn.modelNumber,
