...
def upload_profile(data: UploadProfile, uid: str = Depends(auth.get_current_user_uid_supabase)):
    if data.duration < 10:
...
def upload_profile(file: UploadFile, uid: str = Depends(auth.get_current_user_uid_supabase)):
